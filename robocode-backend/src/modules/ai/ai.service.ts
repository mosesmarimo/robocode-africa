...
import type { ValidateInput, AiResult } from "./dto";
...
export class AiService {
  ...
  private async runChat(system: string, input: ValidateInput): Promise<AiResult> {
  ...
  validateCircuit(input: ValidateInput): Promise<AiResult> {
    return this.runChat(VALIDATE_SYSTEM, input);
  }

  describeCircuit(input: ValidateInput): Promise<AiResult> {
    return this.runChat(DESCRIBE_SYSTEM, input);
  }
}
