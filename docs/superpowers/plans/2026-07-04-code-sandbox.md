...
- Security defaults for every container: `--rm -i --network=none --cap-drop=ALL --security-opt=no-new-privileges --user 65534:65534 --read-only --tmpfs /work:rw,exec,size=64m --tmpfs /tmp:rw,noexec,size=32m --pids-limit=128 --cpus=1`. Node hard-kills the docker process at the wall timeout.
